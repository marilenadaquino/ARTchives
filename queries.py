...
# TO BE CHANGED --the URI of the historian not the graph
def getHistorians():
...
def getKeepers():
	...
	results = sparql.query().convert()
	for result in results["results"]["bindings"]:
		records.add( (result["keeper"]["value"], result["nameKeeper"]["value"].lstrip().rstrip() ))
	return records
...
def getHistorian(artHistorianURI):
	...
	queryHistorian = """ 
		...
		WHERE {
			...
			{ 	
				...
				?collection rdf:type wd:Q9388534 ; wdp:P170 <"""+artHistorianURI+"""> ; rdfs:label ?collectionLabel .
	...
	for result in results["results"]["bindings"]:
		# S_CREATOR_1
		if result.has_key('artHistorianLabel'):
		...
		# S_CREATOR_2
		if result.has_key('birthDate') and result.has_key('deathDate'):
			data['S_CREATOR_2'] = result["birthDate"]["value"]+'-'+result["deathDate"]["value"]
		elif result.has_key('birthDate') and not result.has_key('deathDate'):
			data['S_CREATOR_2'] = result["birthDate"]["value"]+'-'
		elif result.has_key('deathDate') and not result.has_key('birthDate'):
			data['S_CREATOR_2'] = '-'+result["deathDate"]["value"]

		# S_CREATOR_3
		if result.has_key('roleLabel'):
		...
		# S_CREATOR_4
		if result.has_key('countryCreatorLabel'):
		...
		# S_CREATOR_5
		if result.has_key('bio'):
			data['S_CREATOR_5'] = result["bio"]["value"]

		if result.has_key('bioSubjectLabel'):
		...
		# S_CREATOR_6
		if result.has_key('artBiblioRefLabel'):
		...
		# S_CREATOR_7
		if result.has_key('otherbiblioRefLabel'):
		...
		# S_COLL_2
		if result.has_key('collectionLabel'):
		...
		if result.has_key('scopeSubjectLabel'):
...
def getKeeper(keeperURI):
	...
	for result in results["results"]["bindings"]:
		if result["keeperLabel"]["value"]:
			...
			data[fieldID][keeperURI.rsplit('/', 1)[-1]] = result["keeperLabel"]["value"]
		...
		# S_KEEPER_3
		if result.has_key('cityKeeperLabel'):
		...
		# S_KEEPER_4
		if result.has_key('districtKeeperLabel'):
		...
		# S_KEEPER_5
		if result.has_key('countryKeeperLabel'):
		...
		# S_KEEPER_8
		if result.has_key('websiteKeeper'):
...
def getData(graph):
	...
	WHERE {
		...
		{ 	
			...
			OPTIONAL {	?collection art:hasAccessConditions ?access . ?access rdfs:label ?accessLabel . }
	...
	for result in results2["results"]["bindings"]:
		...
		# S_COLL_3
		if result.has_key('earlyDateCollection') and result.has_key('lateDateCollection'):
			data['S_COLL_3'] = result["earlyDateCollection"]["value"]+'-'+result["lateDateCollection"]["value"]
		elif result.has_key('earlyDateCollection') and not result.has_key('lateDateCollection'):
			data['S_COLL_3'] = result["earlyDateCollection"]["value"]+'-'
		elif not result.has_key('earlyDateCollection') and result.has_key('lateDateCollection'):
			data['S_COLL_3'] = '-'+result["lateDateCollection"]["value"]

		# S_COLL_4
		if result.has_key('extent'):
			data['S_COLL_4'] = result["extent"]["value"]

		# S_COLL_5
		if result.has_key('arrangement'):
			data['S_COLL_5'] = result["arrangement"]["value"]

		# S_COLL_6
		if result.has_key('mainTypeLabel'):
		...
		# S_COLL_7
		if result.has_key('otherTypeLabel'):
		...
		# S_COLL_9
		if result.has_key('history'):
			data['S_COLL_9'] = result["history"]["value"]

		# S_COLL_10
		if result.has_key('acquisitionTypeLabel'):
		...
		# S_COLL_11
		if result.has_key('acquisitionYear'):
			data['S_COLL_11'] = result["acquisitionYear"]["value"]

		# S_COLL_12
		if result.has_key('locationLabel'):
		...
		# S_COLL_13
		if result.has_key('accessLabel'):
			data['S_COLL_13'] = result["accessLabel"]["value"]

		# S_COLL_14
		if result.has_key('reproductionLabel'):
		...
		# S_COLL_15
		if result.has_key('findingAid'):
			data['S_COLL_15'] = result["findingAid"]["value"]

		# S_COLL_16
		if result.has_key('standardLabel'):
		...
		# S_COLL_17
		if result.has_key('biblioCollLabel'):
		...
		# S_SUBJ_6
		if result.has_key('artworkLabel'):
